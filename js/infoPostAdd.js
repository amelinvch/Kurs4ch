...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const remainingCharsInput = document.getElementById(
        'my-input-remaining-chars'
    );
    const remainingCharsTextArea = document.getElementById(
    ...
    function printBlockPhoto() {
        const nemPhoto = document.createElement('div');
        nemPhoto.className = 'add_photos';
        nemPhoto.innerHTML = `
			<button class='btn_add_photos'>
			...
			<input class="file_input_photos" type="file" hidden>
		`;
        popa.append(nemPhoto);
        return nemPhoto;
    }
    printBlockPhoto();
    ...
    for (const btn of btnAddPhotos) {
        ...
        btn.addEventListener('click', () => {
            if (countClick < maxAddPhoto) {
                ...
                printBlockPhoto();
                dropPhotoAdd = document.querySelectorAll('.add_photos');
                countClick++;
            }
        });
    }
    ...
    function showFile(filePhotos) {
        ...
        if (validExtensions.includes(fileType)) { //Если файл изображение
            ...
            fileReader.onload = () => {
                ...
                const imgTag = `<img src="${fileURL}" alt="">`; //создание тега img и передача в атрибута src
                dropPhotoAdd[countClick].innerHTML = imgTag; //добавление созданного тега img внутри контейнера dropPhotoAdd
                array.push(imgTag);
            };
    ...
    function inputText() {
        ...
        for (const item of field) {
            if (item.value && validForm(nameAccount, namePattern) &&
                validForm(nameEmail, numberPattern) &&
                validForm(namePhone, emailPattern)) {
